...
async function encryptedPayload(
  ...
  const hkdfKey = hkdf(
    ...
    fileKey,
    nonce,
    Buffer.from(hkdfBodyMessage, "utf8"),
    hkdfKeyLengthBytes
  );
...
export async function decryptAge(
  ...
  if (version !== ageVersion) {
    throw Error(
      `The payload version ${version} is not supported, only ${ageVersion}`
    );
  }

  const fileKey = await unwrapFileKey(encryptedPayload.header.recipients);
  const header = sliceUntil(payload, "---");
  const expectedMac = unpaddedBase64Buffer(
    createMacKey(fileKey, headerMacMessage, header)
  );
  ...
  const nonce = Buffer.from(
    encryptedPayload.body.slice(0, bodyHkdfNonceLengthBytes)
  );
