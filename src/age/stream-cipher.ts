...
export class STREAM {
  static seal(plaintext: ui8a, privateKey: ui8a): Uint8Array {
    ...
    for (let chunk64kb = 1; chunk64kb <= chunks; chunk64kb++) {
      ...
      const output = ciphertext.subarray(
        start * ENCRYPTED_CHUNK_SIZE,
        end * ENCRYPTED_CHUNK_SIZE
      );
  ...
  incrementCounter() {
    if (this.counter == COUNTER_MAX) {
      throw new Error(
        "Stream cipher counter has already hit max value! Aborting to avoid nonce reuse - tlock only supports payloads up to 256TB"
      );
    }
